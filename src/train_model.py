...
from .callbacks import KeepMaxCheckpoints
...
def train_model(config: dict, strategy):
    ...
    checkpoint_config = {
        'filepath': os.path.join(checkpoint_dir, 'model_{epoch}'),
        'save_best_only': True,
    ...
    keep_max_callback = KeepMaxCheckpoints(**keep_max_config)

    extra_callbacks = config.pop('extra_callbacks', [])

    callbacks = [tensorboard_callback, checkpoint_callback, keep_max_callback] + extra_callbacks
